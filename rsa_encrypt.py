...
try:
    ...
    encrypted = rsakey.encrypt(key, None)[0].encode('base64')
    print "\n\n" + encrypted

    # Hash the key to produce a key for assymetric encryption
    ehash = SHA256.new()
    ehash.update(key)
    e_key = ehash.digest()
    #e_key = hashlib.sha256(key).digest()

    # Create the initialization vector
    iv = Random.new().read(AES.block_size)

    # Encrypt the message using AES 256 with the hash key 
...
req = urllib2.Request('http://54.68.56.132/rsa_decrypt.php')

data = {'ciphertext':ciphertext.encode('base64'),
        'iv':iv.encode('base64'),
        'encrypted':encrypted,
        }
